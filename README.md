...
* [`src/task_tracker/styles.cljs`](src/task_tracker/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/task_tracker/`](src/task_tracker/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/task_tracker/core.cljs): contains the SPA entry point, `init`
* [`test/task_tracker/`](test/task_tracker/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `task-tracker` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/task_tracker/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns task-tracker.example
  (:require [task-tracker.config :as config])
